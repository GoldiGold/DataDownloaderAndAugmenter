...
data_dir = os.path.join(consts.DATASET_DIR)
...
def update_goldi_files(case_idx):
    ...
    brain_mask_file = os.path.join(data_dir, consts.SUB_DIR_DICT['brain'], case_idx, consts.NEW_BRAIN_NAME)
    gen_mask_file = os.path.join(data_dir, consts.SUB_DIR_DICT['gen'], case_idx, consts.MASK_NAME)

    t1_out_dir = os.path.join(data_dir, 'T1w-1.25')
    gen_mask_out_dir = os.path.join(data_dir, 'Generated-Masks-1.25')
    ...
    os.makedirs(gen_mask_out_dir, exist_ok=True)
...
if __name__ == "__main__":
    ...
    brain_ids = [str(i) for i in os.listdir(data_dir) if i.isdigit()]  # if this is the correct syntax
    # cases = [line.rstrip() for line in fp.readlines()]

    if num_procs <= 1:
        # Serial execution
        for case_idx, case in enumerate(brain_ids[:10]):
            change_spacing_4D(case_idx)
    else:
        # Parallel execution
        jobs = []
        for case_idx, case in enumerate(brain_ids[:10]):
            p = multiprocessing.Process(target=change_spacing_4D, args=(case_idx))
            jobs.append(p)
